...
void AccelStepper::step1(uint8_t step)
{
	...
	if (PulsePin_state == 1)
		setOutputPins(_direction ? 0b11 : 0b01); // step toggle
	else
		setOutputPins(_direction ? 0b10 : 0b00); // step toggle
	
	// Caution 200ns setup time
	// Delay the minimum allowed pulse width
	//delayMicroseconds(_minPulseWidth);
	//setOutputPins(_direction ? 0b00 : 0b01); // step LOW

	/*
	
	
    setOutputPins(_direction ? 0b11 : 0b10); // step HIGH
    // Caution 200ns setup time 
    // Delay the minimum allowed pulse width
    delayMicroseconds(_minPulseWidth);
    setOutputPins(_direction ? 0b00 : 0b01); // step LOW
